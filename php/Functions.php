...
class Functions
{
    ...
    {
        $dir = rtrim(plugin_dir_path(FM_SCHEMA_FILE), '/');
    ...
    {
        $dir = rtrim(plugin_dir_url(FM_SCHEMA_FILE), '/');
        if (! empty($path) && is_string($path) ) {
            $dir .= '/' . ltrim($path, '/');
        }
        return $dir;
    ...
    public static function sanitize_attribute( $attributes, $attribute, $type = 'text' )
    {
        if (isset($attributes[ $attribute ]) ) {
            ...
            case 'default':
                return new \WP_Error('alerts_dlx_unknown_type', __('Unknown type.', 'fm-schema'));
            }
        }
        return new \WP_Error('alerts_dlx_attribute_not_found', __('Attribute not found.', 'fm-schema'));
    }
}
